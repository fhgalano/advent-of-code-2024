...
fn main() {
    let mut values = parse_values(Path::new("value_input"));
    ...
    for l in values.iter_mut() {
        ...
        if !applicable_rules.iter().map(|x| apply_rule(l, *x)).all(|n| n) {
            while !applicable_rules.iter().map(|x| apply_rule(l, *x)).all(|n| n) {
                println!("bad {:?}", l);
            };
            big_boi += l[l.len() / 2]
        }
